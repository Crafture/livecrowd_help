...
window.addEventListener("DOMContentLoaded", function () {
  ...
  window.copyAnswer = async function (answerHTML) {
    try {
      // Build a plain-text fallback by stripping tags in a temp element
      ...
      alert("Answer copied to clipboard (rich text)!");
    } catch (err) {
  ...
  function loadMoreContent() {
    ...
    isLoading = true;
    fetch(
      `/load_remaining_faqitems/?offset=${currentOffset}&limit=${loadLimit}`,
    )
      ...
      .catch((error) =>
        console.error("Error loading additional FAQ items:", error),
      )
      .finally(() => {
        isLoading = false;
      });
